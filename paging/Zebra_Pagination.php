...
//            return htmlspecialchars($this->_base_url_path . (!empty($query) ? '?' . http_build_query($query) : ''));
            // make sure the returned HTML is CMSimple compliant
            return htmlspecialchars($this->_base_url_path . (!empty($query) ? '?' . cmsimple_http_build_query($query) : ''));
...
function cmsimple_http_build_query($query)
{
    $res = array();
    foreach ($query as $key => $val) {
        $res[] = urlencode($key) . (!empty($val) ? '=' . urlencode($val)
: '');
    }
    return implode('&', $res);
}
